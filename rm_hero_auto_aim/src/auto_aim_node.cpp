#include "rm_hero_auto_aim/auto_aim_node.hpp"
...
namespace rm_hero_auto_aim
{
    AutoAimNode::AutoAimNode(const rclcpp::NodeOptions &options)
    {
        ...
        std::shared_ptr<rm_hero_auto_aim::ArmorDetector> detector = std::make_shared<rm_hero_auto_aim::ArmorDetectorSVM>(node_);
        auto_aim_algo_ = std::make_shared<rm_hero_auto_aim::AutoAimAlgo>(node_, camera_k, cam_info.d, detector);
        auto_aim_algo_->setTargetColor(!is_red);
...
#endif // RM_DEBUG_MODE 
    // 姿态四元数
        curr_pose_ = Eigen::Quaterniond(q.w,
                                        q.x,
                                        q.y,
                                        q.z);
    ...
    {
        // 0x00,休眠模式，0x01:自动射击模式，0x02：自动瞄准模式（不发子弹）,0x03,测试模式,不控制， 0x04,击打哨兵模式.
...
RCLCPP_COMPONENTS_REGISTER_NODE(rm_hero_auto_aim::AutoAimNode)